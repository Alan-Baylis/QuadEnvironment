...
public class MeshGenerator : MonoBehaviour {
    ...
    List<Color> colors;
    List<int> triangles;
    public void GenerateMesh(int[,] map, float squaresize, Vector3 start)
    {
        verticies = new List<Vector3>();
        triangles = new List<int>();
        colors = new List<Color>();

        SquareGrid squareGrid = new SquareGrid(map, squaresize, start);
    ...
    void AssignVerticies(Node[] points)
    {
        for(int i = 0; i < points.Length; i ++)
        {
            if(points[i].vertexIndex == -1)
            {
                ...
                verticies.Add(new Vector3(points[i].position.x,points[i].position.y,1200));
    ...
    public class SquareGrid
    {
        ...
        public SquareGrid (int[,] map, float squareSize, Vector3 start)
        {
            ...
            for(int x = 0; x < nodeCountX; x++)
            {
                ...
                {
                    Vector3 pos = new Vector3(x * squareSize + squareSize / 2, y * squareSize + squareSize / 2,start.z);
	...
	void Start () {
        GetComponent<MeshRenderer>().material.shader = Shader.Find("Particles/Alpha Blended");
    }
